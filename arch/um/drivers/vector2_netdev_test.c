...
static void
vector2_netdev_checksum_features_follow_config_test(struct kunit *test)
{
	struct um_vec2_dev *vdev;
	struct net_device *dev;

	vdev = vector2_netdev_test_alloc_vdev(test, 4);
	dev = vector2_netdev_test_alloc(test, vdev);

	KUNIT_EXPECT_FALSE(test, !!(dev->features & NETIF_F_RXCSUM));
	KUNIT_EXPECT_FALSE(test, !!(dev->hw_features & NETIF_F_HW_CSUM));
	KUNIT_EXPECT_FALSE(test, !!(dev->hw_features & NETIF_F_RXCSUM));

	free_netdev(dev);

	vdev = vector2_netdev_test_alloc_vdev(test, 5);
	vdev->cfg.csum = true;
	vdev->cfg.gso = true;
	vdev->cfg.gro = true;
	dev = vector2_netdev_test_alloc(test, vdev);

	KUNIT_EXPECT_TRUE(test, !!(dev->hw_features & NETIF_F_HW_CSUM));
	KUNIT_EXPECT_TRUE(test, !!(dev->features & NETIF_F_RXCSUM));
	KUNIT_EXPECT_FALSE(test, !!(dev->hw_features & NETIF_F_RXCSUM));
	KUNIT_EXPECT_TRUE(test, !!(dev->features & NETIF_F_GRO));
