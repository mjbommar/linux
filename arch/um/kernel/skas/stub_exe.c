...
static void stub_setup_process(void)
{
	/* set a nice name */
...
static void stub_read_init_data(struct stub_init_data *init_data)
{
	unsigned long res;
...
static void stub_prepare_control_fd(const struct stub_init_data *init_data)
{
	/* In SECCOMP mode, FD 0 is a socket kept for FD passing. */
...
static void stub_map_code_data(const struct stub_init_data *init_data)
{
	unsigned long res;
...
static void stub_close_extra_fds(const struct stub_init_data *init_data)
{
	unsigned long res;
...
static void stub_setup_signal_stack(const struct stub_init_data *init_data)
{
	struct stub_sigaltstack stack = {
...
static void stub_install_sigaction(int signal, struct stub_sigaction *sa,
				   int exit_code)
{
	unsigned long res;

	res = stub_syscall4(__NR_rt_sigaction, signal, (unsigned long)sa, 0,
			    sizeof(sa->sa_mask));
	if (res != 0)
		stub_syscall1(__NR_exit, exit_code);
}

static void stub_setup_signal_handlers(const struct stub_init_data *init_data)
{
	struct stub_sigaction sa = {
	...
	if (!init_data->seccomp) {
		...
		sa.sa_mask = 0;
		stub_install_sigaction(SIGSEGV, &sa, 14);
	} else {
		/* SECCOMP mode uses rt_sigreturn, need to mask all signals */
		sa.sa_mask = ~0ULL;
		stub_install_sigaction(SIGSEGV, &sa, 15);
		stub_install_sigaction(SIGSYS, &sa, 16);
		stub_install_sigaction(SIGALRM, &sa, 17);
		stub_install_sigaction(SIGTRAP, &sa, 18);
		stub_install_sigaction(SIGILL, &sa, 19);
		stub_install_sigaction(SIGFPE, &sa, 20);
	}
}

static void stub_install_seccomp_filter(const struct stub_init_data *init_data)
{
...
static void stub_enter_monitor(const struct stub_init_data *init_data)
{
	/*
	 ...
	 */
	if (init_data->seccomp) {
		stub_install_seccomp_filter(init_data);
...
static noinline void real_init(void)
{
	struct stub_init_data init_data;

	stub_setup_process();
	stub_read_init_data(&init_data);
	stub_prepare_control_fd(&init_data);
	stub_map_code_data(&init_data);
	stub_close_extra_fds(&init_data);
	stub_setup_signal_stack(&init_data);
	stub_setup_signal_handlers(&init_data);
	stub_enter_monitor(&init_data);
}
