 ...
 * inserted at hot paths by the um_on_*() helpers there. When a
 * gate is off the call site is a 5-byte NOP; when on it becomes
 * a JMP into one of the __um_* slow-path functions defined below.
 *
 * For B-02 each slow-path stub bumps a per-gate counter. Workstream
 * C will replace individual stubs with real consumers (ftrace
 * syscall tracer, kcov, KFENCE sampling, etc.) without modifying
 * the gate call sites or this file's public surface.
 */
...
DEFINE_STATIC_KEY_FALSE(um_hook_sanitize_paranoid);
EXPORT_SYMBOL_GPL(um_hook_sanitize_paranoid);

/* --- Hit counters --------------------------------------------------- */

static atomic64_t um_hook_hits[UM_HOOK__COUNT];
...
static const char * const um_hook_names[UM_HOOK__COUNT] = {
	...
	[UM_HOOK_SANITIZE_PARANOID]  = "sanitize_paranoid",
};
...
static struct static_key_false * const um_hook_keys[UM_HOOK__COUNT] = {
	...
	[UM_HOOK_SANITIZE_PARANOID]  = &um_hook_sanitize_paranoid,
};

void um_hook_stats_inc(enum um_hook_id id)
{
	if (likely((unsigned int)id < UM_HOOK__COUNT))
		atomic64_inc(&um_hook_hits[id]);
}
...
u64 um_hook_stats_read(enum um_hook_id id)
{
	if ((unsigned int)id >= UM_HOOK__COUNT)
		return 0;
	return (u64)atomic64_read(&um_hook_hits[id]);
}
