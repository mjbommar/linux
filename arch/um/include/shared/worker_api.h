...
/*
 * UML per-mm host worker process API.
 *
 ...
 *
 * With WORKER_PROCESS=n, one stub child per mm runs in the spawner's
 * VA space. WORKER_PROCESS=y splits each mm into its own worker
 * process.
 *
 * The IPC wire format between spawner and worker is in
 * arch/um/include/shared/worker_ipc.h.
 */
...
struct mm_struct;
struct mm_id;
/* Opaque to spawner code; defined in arch/um/os-Linux/spawner.c. */
struct um_worker;
struct uml_pt_regs;
struct worker_msg;
 ...
 * backend mm_create path when CONFIG_UM_WORKER_PROCESS=y.
 *
...
/*
 * Bring up the per-mm worker and its stub child.
 ...
 * so the caller can fall back to the in-spawner path.
 */
int worker_alloc_stub_for_mm(struct mm_struct *mm, struct mm_id *id_out);

/*
 * Send one worker_msg over the per-mm IPC socket. Returns 0 on success
 * or a negative errno (-ENODEV if the mm has no worker; -EIO on short
 * write).
 */
 ...
 * trap_user.c::seccomp_vcpu_run respectively. The per-iteration shape
 * matches the WORKER_PROCESS=n trap loop: one trap per call. The
 * post-trap signal dispatch (handle_syscall etc.) runs on the
 * originating guest task's current, no extra wakeup.
 *
 * Looks up the worker via current->mm so host-built backend code can
 * call this without pulling in the kernel mm types. Returns -ENODEV
 * if current has no mm or no worker (caller falls back to in-spawner
...
/*
 * Worker IPC loopback check entry point. Spawns a throwaway worker, drives
 * the STUB_ALLOC_REQ -> WRITE_REGS -> RETURN_VALUE -> WRITE_REGS_ACK
 * round-trip, verifies the sentinel echoes, reaps.
 */
int worker_ipc_selftest(void);

#else /* !CONFIG_UM_WORKER_PROCESS */

static inline int spawner_init(void)
{
	return 0;
}
...
static inline int spawn_worker_for_mm(struct mm_struct *mm)
{
	return 0;
}
...
static inline int worker_ipc_selftest(void)
{
	return -ENODEV;
}
