 ...
 *
 * Uses UML's existing SIGSYS + futex + stub-state trap path.
 *
 ...
 * dispatch the kernel-side handler before returning. The function owns
 * its own per-mm turnstile and per-iteration state so the caller's loop
 * body is empty.
 *
 * This host-built file can call libc syscall() and memcpy() directly,
 * matching the existing arch/um/os-Linux/skas/ pattern.
 */
...
void seccomp_vcpu_run(struct uml_pt_regs *regs)
{
	...
	/*
	 * Under WORKER_PROCESS=y the per-mm worker owns the stub child
	 * (cloned CLONE_VM into the worker, not the spawner).
	 * worker_drive_vcpu_run ships VCPU_RUN over the per-mm IPC socket;
	 * the worker runs the futex round-trip against the shared
	 * (memfd, MAP_SHARED) stub_data page and replies VCPU_DONE once the
	 * stub re-traps. Inside that window the spawner-side guest task
	 * answers any SYSCALL_REQ the stub trapped on, running handle_syscall
	 * under its real `current`. The spawner still owns set_stub_state /
	 * get_stub_state and any sendmsg-with-SCM_RIGHTS prelude; see
	 * worker_drive_vcpu_run.
	 *
	 * Returns -ENODEV when current->mm has no worker; falls back to
	 * the in-spawner wait_stub_done_seccomp path.
	 */
