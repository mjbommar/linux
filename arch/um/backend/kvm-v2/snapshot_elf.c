...
/*
 * KVM-backend snapshot ELF64-core export (#181 — snapshot v2 to disk).
 *
 * Given a captured `struct kvm_v2_snapshot`, write it to a caller-
 * supplied file descriptor as an ELF64 ET_CORE-shaped file that gdb,
 * `readelf -n`, and crash(8) can parse without bespoke loaders.
 *
 * The on-disk shape mirrors the design memo
 * Documentation/virt/uml/redesign/08-future-phases/02-snapshot-to-disk.md:
 *
 *   ELF64 header (e_type = ET_CORE)
 *   PT_NOTE phdr  ──► NT_PRSTATUS (gdb-compatible x86_64 prstatus,
 *                                  with pr_reg filled from snap->regs)
 *                     NT_FPREGSET (legacy 512-byte i387 area pulled
 *                                  from the snapshot's xsave region)
 *                     NT_X86_XSTATE (full XSAVE area, sized to match
 *                                    kvm_xsave's static 4 KB region)
 *                     UML private note (n_name = "UML", n_type ==
 *                                       KVM_V2_NT_UML_STATE) holding
 *                                       sregs/xcrs/events/MSR list +
 *                                       a compact descriptor table.
 *   PT_LOAD × N    ──► one per memslot with data captured by Phase 3,
 *                      mapped at p_vaddr = guest_phys_addr so gdb's
 *                      `x` command works against guest physical
 *                      addresses directly. Metadata-only memslots
 *                      (data == NULL — Phase 3 graceful degradation
 *                      under tight vmalloc) are skipped.
 *
 * NOTE: gdb is happy to load a core file that has only NT_PRSTATUS +
 * a PT_LOAD; the other notes light up additional inspection paths
 * (`info xstate` from python helpers, `x/4xg $rsp` against guest RAM,
 * etc). crash(8) recognises ET_CORE without requiring vmlinux symbols
 * — it walks the PT_LOAD ranges and shows the kernel banner string
 * from physmem if it's there.
 *
 * Build is gated on CONFIG_UM_BACKEND_KVM_V2: the per-vCPU snapshot
 * primitives live in snapshot.c which is itself
 * obj-$(CONFIG_UM_BACKEND_KVM_V2). The ELF export TU links into the
 * same module so the ELF-side debugfs trigger ships together with the
...
/*
 * UML-private PT_NOTE n_type. Vendor name "UML" + ordinal 1 per memo
 * 02-snapshot-to-disk §"Layout". The numeric space starts at
 * 0x554d4c00 ('U'|'M'|'L'|0); we use the lowest available ordinal for
 * the consolidated "UML kvm-v2 special state" payload.
 *
 * Anyone adding a new UML note type goes through the design memo
 * (08-future-phases/02-snapshot-to-disk.md §"What this unlocks") so
 * the numbering remains stable; bumping the ordinal is the supported
 * evolution path.
 */
 ...
 * reader can iterate without re-reading the index list out of an
 * embedded array of variable length.
 *
 * Memslot descriptors are dumped here too so the reader can pair
 * each PT_LOAD with its (slot_id, gpa, host_va, flags) without
 * re-deriving from the phdr table. This costs ~24 B / slot; cheap
 * for the ~handful of slots a real UML instance has.
 */
...
struct kvm_v2_uml_state_msr {
	__u64 index;		/* MSR index — widened from __u32 to ease
				 * fixed-stride parsing on the python side.
				 */
	__u64 value;
};
 ...
 * 4 KB (1024 × __u32) shape KVM_GET_XSAVE returns; gdb reads the
 ...
 * named path as ET_CORE. Used by the operator-facing umlctl wrapper +
 * by the snapshot-elf-roundtrip selftest.
 *
 * Why "capture fresh + write" rather than "write the last in-memory
 * snapshot": the in-kernel surface keeps only the lifetime the
 * Phase 3 KUnit fixture and the Phase 5 bench harness allocate.
 * Exposing a long-lived "most recent snapshot" handle would mean
 * persistent state across debugfs writes — needlessly complicates
 * the lifecycle. capture-on-write is stateless and matches the
 * mode-B (application-consistent) story from memo
