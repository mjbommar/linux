...
/*
 * UML backend v2 (KVM) — byte-shape KUnit suite (memo 26 §I.2).
 *
 * Locks the SDM-prescribed byte layouts that v2's exception/syscall
 * machinery depends on. These are pure data tests — no host syscalls,
 * no /dev/kvm, no vCPU state. They re-derive the byte offsets from
 * Intel SDM Vol.3 §3.4.5 (segment descriptors), §6.14.1 (gate
 * descriptors), §6.14.5 (IDT-pushed exception frame), and assert
 * that the layout v2 production code reads/writes against matches.
 *
 * Catches:
 *   - IST frame off-by-8 (the E.5 boot-smoke bug — `off = has_error_code
 *     ? 48 : 40` indexed RIP at the error-code slot)
 *   - GDT segment-descriptor encoding shifts (DPL/L/G/P bits in the
 *     wrong byte)
 *   - IDT gate-descriptor encoding shifts (offset_mid vs offset_high
 *     swap, type/dpl bits out of position)
 *   - sysretq trampoline byte sequence ("48 0f 07")
 *
 * The structure of these tests deliberately does NOT call into v2's
 * static helpers — the assertions verify the byte LAYOUT against the
 * SDM independently, so a test failure means either v2's code or the
 * test got the layout wrong, and a re-derivation from the SDM
 * resolves the disagreement. Running both KUnit suites together is
...
static void test_ist_frame_layout_with_error_code(struct kunit *test)
{
	...
	/* Lay out an SDM-canonical with-error-code frame. */
	...
	*(u64 *)(top -  8) = 0x23ULL;			/* SS = USER_DS */
	...
	/*
	 * E.5 bug regression check: ensure top - 40 (the RIP-anchor)
	 * does NOT land on the error_code slot (top - 48). The pre-E.5
	 * code used `off = has_error_code ? 48 : 40` and read RIP at
	 * `(top - off) + 0` — for has_error_code=true that's top - 48,
	 * the error code, NOT RIP. The fix uses fixed top - 40 + 0.
	 */
...
static void test_ist_frame_layout_without_error_code(struct kunit *test)
{
	 ...
	 * RSP/SS = 5 × 8). RIP at top - 40 (same offset as the
...
/* SDM Vol.3 §3.4.5 — long-mode 8-byte descriptor:                  */
/*   byte 0-1:  limit_low                                            */
/*   byte 2-3:  base_low                                             */
/*   byte 4:    base_mid                                             */
/*   byte 5:    type|S|DPL|P                                         */
...
{
	/*
	 * v1 archive sregs.c:104 + v2 exception.c — kernel CS slot:
...
static void test_idt_pf_stub_bytes(struct kunit *test)
{
	 ...
	 * NOT pop it (SDM Vol.3 §6.14.5), so the stub must `add $8,%rsp`
...
MODULE_DESCRIPTION("UML kvm-v2 byte-shape KUnit tests (memo 26 §I.2)");
MODULE_LICENSE("GPL");
