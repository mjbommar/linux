...
#
# um/vector2-fd-handoff-smoke - validate launcher-owned vector2 fd handoff.
#
# This drives `umlctl up` through the production deployment path: umlctl
# creates the host TAP, opens the TAP queue in the launcher, inherits it into
# the UML process as fd 200, and the guest vector2 driver uses that fd to ping
# the host-side TAP address.
#
...
NAME="v2fd-$SUFFIX"
TAP="v2fd$SUFFIX"
PROBE_TAP="v2fp$SUFFIX"
HOST_IP="10.91.$OCTET.1"
GUEST_IP="10.91.$OCTET.2"
OUT=$(mktemp -d -t vector2-fd-handoff.XXXXXX)
STATE=$(mktemp -d -t vector2-fd-state.XXXXXX)
RUNTIME=$(mktemp -d -t vector2-fd-run.XXXXXX)
UMLFILE="$OUT/vector2-fd-handoff.toml"
...
if ! grep -q "network-fd: open tap=$TAP and inherit fd=200" "$OUT/up.err"; then
	echo "FAIL: umlctl did not report fd 200 TAP handoff"
	cat "$OUT/up.err"
	exit $KSFT_FAIL
fi
if ! grep -q "network: driver=vector2 .* transport=fd host_mode=fd queues=1" \
	"$OUT/up.err"; then
	echo "FAIL: umlctl did not report vector2 fd network plan"
	cat "$OUT/up.err"
	exit $KSFT_FAIL
fi
...
echo "vector2 launcher fd handoff: PASS (tap=$TAP fd=200 guest=$GUEST_IP)"
exit $KSFT_PASS
