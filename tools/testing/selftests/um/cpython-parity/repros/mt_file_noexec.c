// SPDX-License-Identifier: GPL-2.0
/* Multi-thread file-backed mmap of a read-only file. */
#define _GNU_SOURCE
#include <errno.h>
#include <fcntl.h>
#include <pthread.h>
#include <stdio.h>
#include <stdatomic.h>
#include <string.h>
#include <sys/mman.h>
#include <sys/stat.h>
#include <unistd.h>

#define NTHREADS    5
#define ITERATIONS  5000

static atomic_int errors = 0, done = 0;

struct ctx {
	int tid;
	const char *path;
};

static void record_error(void)
{
	atomic_fetch_add(&errors, 1);
}

static void consume_byte(unsigned char value)
{
	asm volatile("" : : "r"(value) : "memory");
}

static void *worker(void *arg)
{
	struct ctx *c = arg;

	for (int i = 0; i < ITERATIONS; i++) {
		struct stat st;
		size_t map_sz;
		int fd;
		void *p;
		unsigned char b;

		fd = open(c->path, O_RDONLY);
		if (fd < 0) {
			fprintf(stderr, "FAIL tid=%d iter=%d open errno=%d\n",
				c->tid, i, errno);
			record_error();
			break;
		}

		if (fstat(fd, &st) < 0) {
			close(fd);
			record_error();
			break;
		}

		map_sz = (st.st_size + 4095) & ~4095UL;
		if (map_sz == 0)
			map_sz = 4096;

		p = mmap(NULL, map_sz, PROT_READ, MAP_PRIVATE, fd, 0);
		close(fd);
		if (p == MAP_FAILED) {
			fprintf(stderr, "FAIL tid=%d iter=%d mmap errno=%d\n",
				c->tid, i, errno);
			record_error();
			break;
		}

		/* Touch the first byte to force fault-in. */
		b = ((unsigned char *)p)[0];
		consume_byte(b);
		munmap(p, map_sz);
	}

	atomic_fetch_add(&done, 1);
	return NULL;
}

int main(int argc, char **argv)
{
	const char *path = argc > 1 ? argv[1] : "/etc/hostname";
	pthread_t t[NTHREADS];
	struct ctx c[NTHREADS];
	int e;

	for (int i = 0; i < NTHREADS; i++) {
		c[i].tid = i;
		c[i].path = path;
		pthread_create(&t[i], NULL, worker, &c[i]);
	}

	for (int i = 0; i < NTHREADS; i++)
		pthread_join(t[i], NULL);

	e = atomic_load(&errors);
	printf("REPRO: %s done=%d errors=%d\n",
	       e ? "ERRORS" : "OK", atomic_load(&done), e);
	return e ? 1 : 0;
}
