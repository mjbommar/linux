...
\centering
\begin{tikzpicture}[
  region/.style={draw=UMLGrid,rounded corners=3pt,align=center,inner sep=8pt,minimum height=1.1cm},
  arr/.style={-{Latex[length=2mm]},thick,UMLMuted},
]
\node[region,fill=UMLGreen!10,minimum width=3.2cm] (user) {guest userspace\\ring 3};
\node[region,fill=UMLBlue!10,minimum width=3.2cm,right=1.1cm of user] (kernel) {\UML{} kernel\\ring 0};
\node[region,fill=UMLPanel,minimum width=7.7cm,below=0.75cm of $(user)!0.5!(kernel)$] (host) {host process + KVM ioctls};
\draw[arr] (user) -- node[above,font=\footnotesize]{LSTAR} (kernel);
\draw[arr] (kernel) -- node[right,font=\footnotesize]{host work} (host);
