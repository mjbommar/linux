...
\begin{frame}{Reader Contract}
\begin{columns}[T,onlytextwidth]
\column{0.48\textwidth}
\begin{block}{Assumed reader}
\begin{itemize}
  \item knows Linux review culture
  \item may know classic \UML{}
  \item does not know this diary
  \item does not know our phase names
\end{itemize}
\end{block}
\column{0.48\textwidth}
\begin{block}{What this deck must do}
\begin{itemize}
  \item state the value proposition
  \item separate inherited work from branch work
  \item explain \KVMvTwo{} without VMM confusion
  \item show evidence and remaining review risk
\end{itemize}
\end{block}
\end{columns}
\end{frame}
...
\begin{frame}{Why Not Existing Tools?}
\scriptsize
\begin{tabularx}{\textwidth}{@{}p{0.19\textwidth}p{0.36\textwidth}X@{}}
\toprule
Option & What it is good at & Why \UML{} still matters \\
\midrule
QEMU/KVM & General virtual machines, device models, booting real images. &
\UML{} is a kernel built as a host process; it is cheaper to instrument
and easier to bind into source-level kernel tests. \\
Firecracker & Minimal production microVMs. & It optimizes a different
surface: cloud isolation and boot footprint, not sanitizer-heavy kernel
research inside \texttt{arch/um}. \\
...
normal user/kernel ring split that matters for syscall and exploit research. \\
...
Functional headline & \KVMvTwo{} has working guest entry, syscall dispatch, exceptions, signals, SMP state repair, and post-gadget performance \\
...
\hspace{1em}
\progress\quad sustained validation
\hspace{1em}
...
\node[label,yshift=-0.36cm] at (b.center) {\KVMvTwo{} two-hour\\validation pilot};
...
\begin{frame}{Eight Answers: Why This Work Exists}
\small
\begin{tabularx}{\textwidth}{@{}p{0.05\textwidth}p{0.28\textwidth}X@{}}
\toprule
\# & Question & Short answer \\
\midrule
1 & Revived value proposition & One upstream Linux tree can produce fast, research, fuzz, sandbox, library, and deterministic profiles. \\
2 & Why not existing tools & They solve adjacent problems; none gives Linux-as-a-process with this instrumentation and syscall fidelity mix. \\
3 & Shared architecture & Backend ops, static-key gates, and profile defconfigs keep variation explicit. \\
4 & Landed vs planned & Core architecture, \KVMvTwo{}, tooling, and gates exist; 24h validation, LTP, syzkaller, and replay remain. \\
\bottomrule
\end{tabularx}
\end{frame}

\begin{frame}{Eight Answers: What Reviewers Should Test}
\small
\begin{tabularx}{\textwidth}{@{}p{0.05\textwidth}p{0.28\textwidth}X@{}}
\toprule
\# & Question & Short answer \\
\midrule
5 & SMP and state & v2 now has an explicit state-home contract and audit trail for per-task, per-vCPU, and per-mm state. \\
6 & Speed beyond micro & Python and stress tiers win on every benchmark host, not only \texttt{getpid}. \\
7 & Upstream bar & Finish sustained validation and split small subsystem series before the large \texttt{arch/um} series. \\
8 & Reviewer challenges & Challenge the data model, validation breadth, upstream slicing, and replay/snapshot claims. \\
\bottomrule
\end{tabularx}
\end{frame}
...
\begin{frame}{Milestone Map, Plain English}
\centering
\begin{tikzpicture}[
...
\node[phase,donep,right=0.08cm of a] (b) {profile\\matrix};
\node[phase,donep,right=0.08cm of b] (c) {KVM v1\\lesson};
\node[phase,donep,right=0.08cm of c] (d) {KVM v2\\core};
\node[phase,donep,right=0.08cm of d] (e) {SMP state\\audit};
\node[phase,donep,right=0.08cm of e] (f) {gadget +\\perf};
\node[phase,progp,right=0.08cm of f] (j) {24h\\validation};
\draw[-{Latex[length=2mm]},thick,UMLMuted] ($(a.south)+(0,-0.35)$) -- ($(j.south)+(0,-0.35)$);
\end{tikzpicture}
\vspace{1em}

\begin{itemize}
  \item Internal phase labels exist in the diary, but reviewers should read the work as milestones.
  \item The current frontier is not a new design idea; it is sustained workload breadth before upstreaming.
\end{itemize}
\end{frame}
...
\node[rung,live,below=0.05cm of l1] (l2) {\textbf{internal contracts} -- KUnit, marshal, byte-shape checks};
...
\node[rung,live,below=0.05cm of l4] (l5) {\textbf{performance gates} -- prevent fast-path regression};
\node[rung,live,below=0.05cm of l5] (l6) {\textbf{real workload boot} -- Python and stress workloads};
\node[rung,prog,below=0.05cm of l6] (l7) {\textbf{sustained validation} -- 24h soak plus Tier 2/3 and LTP curation};
\end{tikzpicture}
\end{frame}

\begin{frame}{Sustained Validation: First Pilot}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
...
\vspace{1em}
\begin{itemize}
  \item This is a pilot for the 24-hour validation bar, not the final upstream gate.
...
\begin{itemize}
  ...
  \item optional AVX-512 follow-up
\end{itemize}
\end{block}
\end{columns}
\end{frame}
...
7 & KVM backend & arch/um + KVM & blocked by sustained validation \\
\bottomrule
\end{tabularx}
\end{frame}

\begin{frame}{What Reviewers Should Challenge}
\small
\begin{columns}[T,onlytextwidth]
\column{0.48\textwidth}
...
\begin{itemize}
  ...
  \item The next credible milestone is not another design memo; it is sustained validation and an upstreamable series plan.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item \texttt{02-workstreams/D-kvm-backend/}: KVM design, v1-to-v2 restart memos, state audit, benchmarks, sustained validation.
  \item \texttt{03-architecture-review-2026-04-27/}: root-cause synthesis for the v1 contract mismatch.
