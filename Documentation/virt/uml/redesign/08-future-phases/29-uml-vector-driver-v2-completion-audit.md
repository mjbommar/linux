...
| KUnit coverage for config, lifecycle, queue, fake host, transport, host-open failure, unwind | `um_vector2_*` KUnit passes 71/71 after fd wrong-type and fd multiqueue unwind coverage | Partial: coverage exists, but more failure injection remains |
...
| Multiqueue TAP/fd KCSAN and distribution | TAP multiqueue works and queue counters move; fd multiqueue core opens contiguous inherited fd ranges under KUnit; `umlctl` fd multiqueue passes live 4-queue smoke and 3/3 gate loop; KCSAN and broader fairness profiles absent | Partial |
...
- `um_vector2_*` KUnit: 71/71 passed;
