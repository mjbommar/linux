...
Findings:
  ...
  - **UDP / syscall-rate / CPU-utilisation:** paced and buffered unpaced
    ...
    host-to-guest UML-PID perf collection in `perf-window.tsv` plus aggregate
    and comparison TSVs.  Steady-state syscall-rate and full CPU-utilisation
...
`perf-window.tsv` and aggregate/comparison rows support that work, and the
privileged perf-stat smoke proves syscall tracepoint collection is available.
