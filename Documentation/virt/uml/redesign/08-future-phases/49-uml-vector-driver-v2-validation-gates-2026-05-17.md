...
Findings:
  ...
  - **UDP / syscall-rate / CPU-utilisation:** paced and buffered unpaced
    ...
    host-to-guest UML process metric deltas in `summary.tsv`; syscall-rate and
    full CPU-utilisation are not measured.
...
support that work, and host-to-guest runs now add UML process deltas from
`umlctl metrics`, but they do not replace `perf stat` syscall-rate data or
