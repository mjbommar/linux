...
Findings:
  ...
  - **UDP / syscall-rate / CPU-utilisation:** paced and buffered unpaced
    ...
    and CPU counters per driver, but steady-state syscall-rate and full
    CPU-utilisation are not yet measured.
...
and host-to-guest UML process deltas from `umlctl metrics` support that work,
and the privileged perf-stat smoke proves syscall tracepoint collection is
available.  They do not replace a steady-state same-throughput syscall-rate
and CPU accounting comparison.  Acceptance is a judgment call on the resulting
