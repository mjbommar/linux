...
Findings:
  ...
  - **UDP / syscall-rate / CPU-utilisation:** paced and buffered unpaced
    1 MiB UDP fixed-byte evidence exists, and 8 MiB guest-to-host UDP passes
    for both drivers.  Larger host-to-guest UDP still misses exact-byte
    completion.  The fixed-byte helper now records per-process endpoint CPU
    seconds plus host-to-guest UML process metric deltas in `summary.tsv`;
