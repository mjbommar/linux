...
Findings:
  ...
  - **UDP / syscall-rate / CPU-utilisation:** paced and buffered unpaced
    1 MiB UDP fixed-byte evidence exists, 8 MiB guest-to-host UDP passes for
    both drivers, and paced 8 MiB host-to-guest UDP passes for both drivers.
    Unpaced 8 MiB host-to-guest UDP still misses exact-byte completion.  The
    fixed-byte helper now records per-process endpoint CPU seconds plus
    host-to-guest UML process metric deltas in `summary.tsv`.  A privileged
    64 KiB host-to-guest TCP perf-stat smoke now records subtree-wide syscall
    and CPU counters per driver, and the helper now has opt-in bounded
    host-to-guest UML-PID perf collection in `perf-window.tsv`.  Steady-state
    syscall-rate and full CPU-utilisation are not yet measured.
...
The follow-up lazy-RX cleanup prepares one RX slot per nonblocking read attempt
instead of preparing the full NAPI budget up front.  KUnit reports 88 pass and
2 trusted-TAP skips, focused fd handoff, fd multiqueue, in-process TAP, and
pool TAP smokes pass, and the normal guest-to-host TCP gate remains green:
legacy vector median 39215.2 Mbps, vector2 median 38960.1 Mbps, ratio 0.993.
The 1 MiB host-to-guest fixed-byte cell still fails the parity bar, but its RX
batch counters no longer show tens of thousands of unused prepared slots.

The follow-up RX checksum feature-alignment slice makes vector2 advertise
fixed RX checksum support when `csum=1`, matching the vnet-header receive path
and legacy vector's TAP behavior.  KUnit reports 89 pass and 2 trusted-TAP
skips, focused fd handoff, fd multiqueue, and in-process TAP smokes pass, and
the fixed-byte diagnostic now records vector2 `rx-checksumming: on [fixed]`
with `vnet_hdr_enabled: 1`.  The normal guest-to-host TCP gate remains green:
legacy vector median 39854.4 Mbps, vector2 median 39441.7 Mbps, ratio 0.990.
The 1 MiB host-to-guest cell remains open.
...
`next` with the fixed-byte bidirectional TCP harness:
  ...
  - host -> guest best observed host-side ratios:
    - 1 MiB: 0.483 in the bidirectional run and 0.579 in a focused four-repeat
      ...
      not close the gap; lazy RX improved the focused best ratio to 0.594 while
      reducing representative RX-slot preparation from 32448 prepared / 906
      received / 31542 released slots to about 1410 prepared / 905 received /
      505 released slots;
    - 8 MiB: 2.002;
    - 32 MiB: 0.901.
...
already; extending the matrix to cover broader UDP + steady-state syscall +
full CPU utilisation is operator time.  The helper-level endpoint CPU columns,
host-to-guest UML process deltas from `umlctl metrics`, and bounded
`perf-window.tsv` rows support that work, and the privileged perf-stat smoke
proves syscall tracepoint collection is available.  They do not replace a
steady-state same-throughput syscall-rate and CPU accounting comparison.
Acceptance is a judgment call on the resulting table — this memo defines the
bars.
