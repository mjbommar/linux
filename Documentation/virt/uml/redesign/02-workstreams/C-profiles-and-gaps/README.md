...
| 04 | [Port kprobes](04-port-kprobes.md) | 4 wk | **partially landed (2026-04-20)** — HAVE_KPROBES (commits 1a-1d) + HAVE_RETHOOK/KRETPROBES (commit 2) in tree; commit 3 HAVE_FUNCTION_GRAPH_TRACER split 3a/3b 2026-04-21 — 3a (strip infrastructure + notrace on generic kthread/smpboot) landed, 3b (actual tracer flip + trampolines) deferred one session pending atomic-context fix in prepare_ftrace_return. See D34's 2026-04-21 addenda. | Yes |
...
| 06 | [Port BPF JIT](06-port-bpf-jit.md) | 3 wk | **landed v1 (2026-04-21)** — option A per D43 fifth-view. UML consumes `arch/x86/net/bpf_jit_comp.c` with UML-local shim headers (`arch/um/include/asm/vsyscall.h`, cpufeature stub, `arch/um/kernel/x86_nops.c`) + a `CONFIG_UM` branch in `reg2pt_regs[]`. `uml/research` profile enables `CONFIG_BPF_SYSCALL=y` + `CONFIG_BPF_JIT=y` out of the box. Post-landing fixes: shim split 48aa69e2d4a7 (uml/fuzz build repair). Option B2 portable-emitter refactor tracked separately as task #68. Two upstream-bound hygiene commits (e2b686c962, 5b95b1bb3e) remain staged in `upstream-patches/bpf-hygiene-v1/`. | Yes |
| 07 | [Port KMSAN](07-port-kmsan.md) | 6 wk | **design-locked (2026-04-22)** — four probes + resolution in D44; upstream RFC strategy in D51. Path forward: one ~20 LOC generic patch (new weak `kmsan_arch_init_early_shadow()` hook + un-static `kmsan_record_future_shadow_range()`, zero behavior change for x86/s390), then 5-6 UML-side commits on top. Implementation-queued behind LKML coordination with Alexander Potapenko. ~1 week of focused work once the generic hook merges. | Last (heaviest) |
