...
| Pool | Pool benchmark thresholds | Present-needs-fix | `memo09-phase3-pool-bench`, `memo09-phase4` | Reduced live-child benchmark passes; full default benchmark still needs RSS and throughput closure or a documented target revision. | Reduced `pool-bench` PASS 5/5 with 146.0 MiB RSS; full `pool-bench` PASS 3/5, FAIL RSS 7,409.4 MiB/200 MiB and throughput 2250/3000, 2026-06-10. |
| Pool | mconsole path synthesis | Partial-needs-fix | `next`, `memo09-phase4` | Keep and redesign/prove a durable per-member socket; a naive child-side rebind experiment panicked before `MEMBER_DONE`. | `pool-exec-smoke` PASS for typed error envelope; `pool-mconsole-path-probe` XFAIL with member alive and socket absent; focused investigation in `2026-06-10-pool-mconsole-exec-investigation.md`, 2026-06-10. |
| Pool | `umlctl exec` via daemon | Present-needs-fix | `next`, `memo09-phase4` | Keep; error envelope validated, but successful guest exec needs both a per-member control socket and a real kernel/daemon exec primitive. | `pool-exec-smoke` PASS for expected failure envelope; mconsole command table has no `exec` verb, 2026-06-10. |
