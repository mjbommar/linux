...
| Record/replay | SIGALRM determinism | Partial/needs-decision | `kvm-v2-snapshot-elf64`, `experiment-path-c` | Define supported tier and implement before declaring complete. UML time-travel clock events are present, and raw time observations now fail closed; SIGALRM injection is still open. | deterministic workload gate. |
...
- KVM v2 experimental record/replay core is present on `next` behind
  ...
  `make ARCH=um -j$(nproc)`, `um_kvm_v2_record` 20/20, `kvm_v2_marshal` 9/9,
