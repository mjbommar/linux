...
| Record/replay | Raw time syscall and RDTSC policy | Present-experimental-fail-closed | `next` | UML vDSO clock helpers route through syscalls. Strict replay rejects raw time syscalls outside the supported set, and KVM v2 sets CR4.TSD while replay is active so direct user timestamp reads fault instead of observing host time outside the log. Replayable raw-time payloads and direct `RDTSCP` runtime evidence remain future work. | `um_kvm_v2_record` strict time-policy case PASS; `kvm-record-smoke` PASS with `live-time=1` and `live-rdtsc=1`, 2026-06-11. |
