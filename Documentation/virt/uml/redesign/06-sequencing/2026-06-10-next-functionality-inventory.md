...
| Pool | `umlctl exec` via daemon | Present-needs-fix | `next`, `memo09-phase4` | Keep; error envelope validated, but successful guest exec still needs a real kernel/daemon exec primitive or replacement transport. | `pool-exec-smoke` PASS for expected failure envelope; current host reaches missing `uml_mconsole(1)` after the member socket fix, and the active mconsole command table has no `exec` verb, 2026-06-10. |
