...
| --- | ------ | ---- | ---- |
| `next` | `bb092119158b` | 2026-06-11 | Authoritative integration target. |
| `origin/next` | `bb092119158b` | 2026-06-11 | Remote tracking ref for `next`. |
...
| Record/replay | Deterministic task-owned workload replay | Present-experimental-core | `next` | The task-owned helper now suppresses the debugfs control `write(2)` from the record log, records only the bounded scalar plus `uname(2)`, `getcwd(2)`, and direct raw-time argument-shape payload workload, replays it from the snapshot-backed log, compares replayed raw-time bytes with the recorded values, and auto-disarms when the cursor reaches the end. This proves the first R/R-1 deterministic workload for the current direct-syscall subset, not whole-system replay. | `KVM_RECORD_TASK: PASS pid=1 entries=393 syscalls=393 same=393 other=0 payload_entries=8 payload_bytes=528 replayed=393`, 2026-06-11. |
...
| Template pause | Kernel template pause entry | Present-validated | `next`, `memo09-phase*`, `fork-server-phase1c` | Historical comparison complete; smoke now finishes with bounded teardown. Vector2 leg skips when guest `vec0` is absent. | `template-pause-smoke` PASS cases 1-3, SKIP case 4, rebuilt `bb092119158b`, 2026-06-11. |
| Template pause | Identity blob layout and apply path | Present-validated | `next`, `memo09-phase2`, `memo09-phase4` | Current layout matches the final launcher contract by source comparison and live pool-member validation. | `template-pause-pool-member-smoke` PASS with identity fd parsed and five timer ticks, rebuilt `bb092119158b`, 2026-06-11. |
| Template pause | Template pause pivot mode | Present-validated | `memo09-phase4`, `next` | Keep; pivot smoke passed on current `next`. | `template-pause-pivot-smoke` PASS, 20 pivots, rebuilt `bb092119158b`, 2026-06-11. |
| Fork server | Fork-on-resume loop | Present-validated | `next`, `memo09-phase4` | Keep the current production path; the corrected smoke drives the second take and default stress now passes. | `template-pause-fork-smoke` PASS, child PIDs distinct and master resume cycles=2; `template-pause-fork-stress` PASS, 555 iterations, median 18.0 ms, 555/555 clean identities, rebuilt `bb092119158b`, 2026-06-11. |
| Fork server | Child PID reporting through identity memfd | Present-validated | `next`, `memo09-phase4` | Keep; validate in pool take and pool-member smoke. | `pool-serve-smoke` PASS and `template-pause-pool-member-smoke` PASS, rebuilt `bb092119158b`, 2026-06-11. |
...
| Pool | Direct `umlctl pool spawn` | Present-validated | `next`, `memo09-phase*` | Keep. | `pool-spawn-smoke` PASS, rebuilt `bb092119158b`, 2026-06-11. |
| Pool | Daemon `pool serve` | Present-validated | `next`, `memo09-phase4` | Keep and continue through warm-pool completion. | `pool-serve-smoke` PASS, rebuilt `bb092119158b`, 2026-06-11. |
| Pool | `pool take` | Present-validated | `next`, `memo09-phase4` | Keep and extend for warm ready members. | `pool-serve-smoke` PASS, rebuilt `bb092119158b`, 2026-06-11. |
| Pool | `pool list/status/destroy/shutdown` | Present-validated | `next`, `memo09-phase4` | Keep. | `pool-spawn-smoke` PASS and `pool-serve-smoke` PASS, rebuilt `bb092119158b`, 2026-06-11. |
| Pool | Warm member pool | Present-validated | `memo09-phase3-pool-bench`, `memo09-phase4`, `next` | Keep the two-mode contract: request-specific takes stay lazy because identity is applied before fork; returned members are live but quiesced until daemon-routed `exec` resumes them; `pool take --ready` consumes daemon-assigned pre-identified members and cannot be combined with caller-supplied identity. No predeclared slot API is required for the current completion claim. | `pool-serve-smoke` PASS with `--min-warm=1`, ready take, replenish, destroy, and shutdown; `pool-exec-smoke` proves resume-before-exec; syzkaller remains on the request-specific lazy take path because it needs deterministic TAP/IP identity, rebuilt `bb092119158b`, 2026-06-11. |
| Pool | Pool benchmark thresholds | Present-validated | `memo09-phase3-pool-bench`, `memo09-phase4` | Full default benchmark now gates memory amplification on PSS while still reporting summed RSS as diagnostic context; this matches the quiesced live-member model where executable/libc/tmpfs pages are intentionally shared. | Full `pool-bench` PASS 5/5: p50 1.8 ms, p99 2.5 ms, `pss.100forks` 139.6 MiB PSS with 509.2 MiB summed RSS and 17.9 MiB private dirty for 100/100 live members, lifecycle drift 0.00%, throughput 3000/3000, rebuilt `bb092119158b`, 2026-06-11. |
| Pool | mconsole path synthesis | Present-validated | `next`, `memo09-phase4` | Keep the master-side bind plus child-side SIGIO rearm model; avoid the rejected child-side rebind experiment that panicked before `MEMBER_DONE`. | `pool-mconsole-path-probe` PASS with member alive, per-member socket present, and rebuilt-kernel `version` reply from `bb092119158b`; focused investigation in `2026-06-10-pool-mconsole-exec-investigation.md`, 2026-06-11. |
| Pool | `umlctl exec` via daemon | Present-validated | `next`, `memo09-phase4` | Keep `exec/1` as the current public ABI: callers send structured argv/env/cwd/timeout to the daemon and receive NDJSON frames. The current mconsole backend deliberately lowers that request through a bounded shell command; `--timeout` requires guest `timeout(1)`. Any stricter kernel transport is future `exec/2` work, not required for the current completion claim. | `pool-exec-smoke` PASS: `/bin/true` exits 0, stdout/stderr capture round-trips, guest exit 7 is preserved without a daemon error, timeout returns code 124 with `timed_out=true`, late stdout is suppressed, no extra guest `sleep` helper leaks, and stale `Unknown command`/missing-host-tool boundaries are rejected. The mconsole readiness wait now follows the requested exec timeout, with a 10-second floor, instead of using a fixed 10-second probe budget. Historical `memo09`/`umlctl-deploy` branches had the same outer RPC/NDJSON intent and no stricter kernel argv transport to import, 2026-06-11. |
| Pool | `umlctl port-forward` | Present-validated | `next`, `memo09-phase4` | Keep and later validate against final network mode. | `pool-port-forward-smoke` PASS, rebuilt `bb092119158b`, 2026-06-11. |
| Pool | Vector2 TAP handoff | Present-validated | `next`, `memo09-phase4`, `umlctl-deploy` | Keep the vector2 TAP reopen path and smoke gate. Per-take pool fd handoff is retired from the current completion claim; current pool takes carry string identity through the identity memfd and reopen vec2 TAP by per-member TAP name. | `vector2-pool-tap-smoke` PASS: per-member TAP/MAC/IPv4 identity visible through daemon exec and one-packet host TAP ping succeeds; source audit confirms `um_template_identity_apply()` calls `um_vec2_tap_reopen_for_pool_member()` before IPv4/route apply; current-head rerun PASS with the rebuilt `umlctl` after mconsole readiness timeout alignment, 2026-06-11. |
| Vector2 | Typed parser | Present-validated | `next` | Keep. | `um_vector2_config` 12/12 and `um_vector2_transport` 8/8 KUnit PASS in the current `um_vector2_*` rerun, 2026-06-11. |
| Vector2 | Queue ownership | Present-validated | `next` | Keep. | Current `um_vector2_*` KUnit PASS with 96 pass, 0 fail, and 2 trusted-TAP skips, including `um_vector2_queue` 9/9, `um_vector2_netdev` 18/18, and `um_vector2_ethtool` 6/6; the same suite covers the lazy-RX, fd/vnet allocation, RX-only IRQ, and TX-stall retry-timer helpers, 2026-06-11. |
| Vector2 | fd backend | Present-validated-needs-long-gates | `next`, `umlctl-deploy` | Keep the launcher-owned inherited-fd path for standalone `umlctl up`; per-take pool fd handoff is not part of the current completion claim. | `vector2-fd-handoff-smoke` PASS with fd 200 handoff and guest `10.91.105.2`; `um_vector2_host_fd` KUnit 14/14 PASS; historical branch grep found the pool SCM_RIGHTS TAP-fd swap deferred rather than implemented, 2026-06-11. |
| Vector2 | tap backend | Present-validated-needs-long-gates | `next`, `umlctl-deploy` | Keep and run networking/Tier 3 gates. | `vector2-inproc-tap-smoke`, `vector2-pool-tap-smoke`, and `vector2-sandbox-audit` PASS with the rebuilt current `umlctl`; bounded KVM-v2/vector2 Tier 3 smoke PASS 2/2 for Django-v2 and FastAPI-v2, but full Tier 3 remains required, 2026-06-11. |
| Vector2 | multiqueue | Present-validated-needs-fairness | `next`, `umlctl-deploy` | Keep the launcher-owned multiqueue fd handoff path; finish fairness/performance gates. | `vector2-fd-multiqueue-smoke` PASS: `umlctl up` creates a multiqueue TAP, opens/inherits fds 200..203, guest metadata reports four queues/fds, `vec2.0` reports four TX queues, guest address `10.92.84.2` is assigned, and one-packet host TAP ping succeeds, 2026-06-11. |
| Vector2 | TX write-ready IRQ suppression | Present-validated-needs-matrix | `next` | Keep RX IRQs for host readability, but do not register permanent TAP/fd write IRQs; use NAPI plus a bounded TX retry timer for no-progress TX polls. This avoids always-write-ready host-fd wakeups on receive-only workloads while preserving transient TX backpressure recovery. | Clean UML rebuild PASS; `um_vector2_*` KUnit 96 pass, 0 fail, 2 trusted-TAP skips, including RX-only IRQ and TX-stall retry-timer coverage; fd handoff, fd multiqueue, in-process TAP, pool TAP, and sandbox audit smokes PASS; vector2 64 KiB guest-to-host TCP sanity PASS; focused 1 MiB host-to-guest default fd/multiqueue rerun improved host-side median ratio from 0.650602 to 0.899069 but still needs broader repeat/fairness/Tier 3/CPU-syscall publication evidence, 2026-06-11. |
| Vector2 | performance benchmark harness | Present-validated-needs-matrix | `next` | Keep `net-bench` as an explicit operator-run gate; expand the remaining P4.3/P4.5 matrix before making replacement claims. | `tools/testing/selftests/um/net-bench` now has a kselftest Makefile, builds `tcp-send`, uses repo-relative defaults instead of developer-local paths, validates wrapper syntax, and captures guest `ip`/route/ethtool diagnostics. Initial TCP run: legacy vector 3/3 PASS median 39805.4 Mbps; vector2 3/3 PASS median 18949.8 Mbps; ratio 0.476 < 0.85. Scatter-gather TX improved vector2 to median 22953.7 Mbps versus legacy 40041.7 Mbps, ratio 0.573. A bounded `sendmmsg()` prototype did not improve the ratio and was not committed. TX/RX NAPI scheduling then fixed the measured guest-to-host regression: `um_vector2_*` KUnit PASS with 87 pass, 0 fail, and 2 trusted-TAP skips, fd handoff/in-process TAP/fd multiqueue smokes PASS, and normal TCP gate PASS with legacy vector median 40080.5 Mbps, vector2 median 37116.0 Mbps, ratio 0.926. Current fixed-byte bidirectional TCP refresh clears guest-to-host at 1/8/32 MiB with ratios 0.966/1.007/0.925 and host-to-guest at 8/32 MiB with ratios 2.002/0.901, but 1 MiB host-to-guest remains below legacy at 0.483 and 0.579 on rerun; a single-queue vector2 rerun reached 0.663. Lazy RX reduces representative 1 MiB host-to-guest RX-slot churn from 32448 prepared / 906 received / 31542 released slots to about 1410 prepared / 905 received / 505 released slots, keeps the normal guest-to-host gate green at ratio 0.993, and improves the focused host-to-guest best ratio only to 0.594. The fd/vnet RX allocation fix closes a framing correctness bug; focused 1 MiB host-to-guest rerun saw vector2 median 0.895 MiB/s versus legacy vector median 0.976 MiB/s, so the small-transfer blocker remains open. The fixed-byte helper now supports UDP; TCP compatibility and vector2 UDP bidirectional smoke pass, and current-HEAD repeat-2 UDP matrices validate buffered unpaced 1 MiB and paced 8 MiB in both directions with vector2 close to or above legacy on host-side MiB/s. It also emits transfer-window `perf-window.tsv`, `perf-window-aggregate.tsv`, and `perf-window-comparison.tsv`; a two-driver two-repeat 1 MiB host-to-guest TCP smoke validated real median/ratio rows, and a follow-up parameter/topology sweep found single-queue fd mode is the best current hint at host-side median ratio 0.810510 and best ratio 0.906732 while `TCP_NODELAY`, smaller sender chunks, and in-process TAP do not close the cell. Suppressing idle TX write IRQs and using a no-progress TX retry timer improved the default fd/multiqueue 1 MiB host-to-guest host-side median ratio to 0.899069, with transfer-window perf ratios lower than legacy vector for syscalls, task-clock, and context switches. The latest explicit guest-to-host TCP `net-bench` rerun passes at ratio 0.958 with legacy vector median 41865.7 Mbps and vector2 median 40118.6 Mbps. The natural seccomp/vector2 Tier 3 soak passes with 740/740 rows over 7221s. Broader repeat/fairness/KVM-v2 Tier 3/CPU-syscall validation remains required. Unpaced 8 MiB host-to-guest UDP and final steady-state CPU/syscall publication coverage remain open. |
| Vector2 | raw/gre/l2tpv3/vde/bess/proxy/hybrid transports | Parser-only-retired-from-runtime-claim | `next`, historical vector branches | Do not count these as implemented runtime transports on `next`; keep GRE/L2TPv3 header helpers under KUnit and require new netdev backends plus live smokes before restoring any runtime claim. | Netdev KUnit guards parser-only transports returning `-EOPNOTSUPP`; GRE/L2TPv3 header-helper KUnit remains present; current `um_vector2_*` KUnit PASS with 96 pass, 0 fail, and 2 trusted-TAP skips, 2026-06-11. |
...
| Vector2 | sandbox mode | Present-validated | `next` | Keep the default-safe audit gate. | `vector2-sandbox-audit` PASS with the rebuilt current `umlctl`: `umlctl gate loop --audit-vector-sandbox` ran a vector2 auto-queue fd boot and reported `PASS=1/1 FAIL=0 TIMEOUT=0` with no forbidden host operations, 2026-06-11. |
| Vector2 | in-process trusted mode | Present-validated | `next` | Keep explicit and document authority. | `vector2-inproc-tap-smoke` PASS with the rebuilt current `umlctl`: `umlctl up` reports TAP/inproc mode without launcher fd inheritance, guest metadata reports TAP/inproc with fd count zero, `vec2.0` has the assigned IPv4 address `10.93.217.2`, and one-packet host TAP ping succeeds, 2026-06-11. |
...
| Launcher | `umlctl up/down/ps/logs` | Present-validated | `next`, `umlctl-deploy` | Keep. | `umlctl-smoke` PASS on rebuilt `bb092119158b`; `cargo fmt --check`, `cargo test --locked` with 215 tests, and `cargo build --locked --bin umlctl` PASS in `tools/uml/uml-launcher`, 2026-06-11. |
| Launcher | deploy configs | Present-validated | `next`, `umlctl-deploy` | Keep current example/profile set; current `umlctl` uses `up/down` plus Umlfile parsing, not a separate deploy subcommand. No useful historical deploy TOML is missing from `next`. | File-set comparison found no example/profile files unique to `umlctl-deploy`; all 22 example Umlfiles now use the current `umlctl up` schema and pass `UML_KERNEL=$PWD/linux umlctl up --dry-run`; all 5 built-in profiles resolve with `umlbuild profile show`, 2026-06-11. |
...
| Selftests | local stress/soak tests | Present-cleaned | `next`, `umlctl-deploy` | Keep under clear local/soak docs and avoid developer-local paths in checked-in templates/defaults. | Soak templates now render helpers through `{{SOAK_DIR}}`, repo builds through `{{REPO_ROOT}}`, and Tier 2 uv through `TIER2_UV_PYTHON`; `run-soak-daemon.sh` and `run-pilot.sh` default to the repo-built debug `umlctl` when present. Bash syntax passes, the soak subtree has no checked-in `/home/mjbommar`/bench-bundle/old-build defaults, and a daemon dry-run rendered memcheck, kbuild-tiny, tier2 uv, and tier3 vector2 templates, 2026-06-11. |
...
| Docs | active UML user docs | Present-cleaned | `next` | Keep non-redesign UML docs focused on current interfaces and limitations, not workstream history. | Normalized ftrace, kprobes, KMSAN, debugfs, section-split, snapshot/forkserver, launcher, APERF/MPERF, and profile docs; `Documentation/virt/uml/review-01/` now contains only compatibility pointers and the historical review content moved under `redesign/07-references/review-01/`; focused scan over non-redesign UML docs now has no standalone workstream/decision/memo/phase-history labels apart from sample log/noise terms, 2026-06-11. |
...
- Daemon-routed pool exec now validates success, stdout/stderr capture, guest
  ...
  kernel argv transport is future `exec/2` work. The mconsole readiness wait
  now follows the requested exec timeout, with a 10-second floor, instead of
  using a fixed 10-second probe budget.
...
- The focused pool/fork/syzkaller regression set passes on the rebuilt
  current-HEAD `bb092119158b` UML binary: template-pause, fork smoke/stress,
...
- The current-head vector2 validation refresh passes on the rebuilt current
  branch: `um_vector2_*` KUnit reports 96 pass, 0 fail, 2 trusted-TAP skips;
  fd handoff, fd multiqueue, in-process TAP, sandbox audit, failed-open, and
  pool TAP smokes pass; the explicit guest-to-host TCP `net-bench` gate passes
  at ratio 0.958; bounded KVM-v2/vector2 Tier 3 smoke passes 2/2 for
  Django-v2 and FastAPI-v2; and the natural seccomp/vector2 Tier 3 soak
  reaches 7221 seconds with 740/740 PASS. Full KVM-v2 Tier 3 and
...
- Vector2 UDP fixed-byte evidence is no longer missing. The performance helper
  ...
  before the success marker. Current-HEAD validation: TCP compatibility smoke
  PASS, vector2 UDP bidirectional 64 KiB smoke PASS, repeat-2 buffered
  unpaced 1 MiB UDP matrix PASS, and repeat-2 paced 8 MiB UDP matrix PASS. In
  the repeat-2 buffered unpaced 1 MiB run, vector2/vector host-side ratios
  were 0.998392 guest-to-host and 1.125627 host-to-guest. In the repeat-2
  paced 8 MiB run with `UML_VECTOR_PERF_UDP_PACE_USEC=20`, vector2/vector
  host-side ratios were 0.992588 guest-to-host and 1.003083 host-to-guest.
  Earlier unpaced 8 MiB host-to-guest evidence still lost bytes before
  exact-byte completion for both legacy vector and vector2, so larger unpaced
  host-to-guest UDP remains bounded rather than accepted, 2026-06-11.
...
- The fixed-byte helper now has opt-in transfer-window perf collection for
  ...
  <pid>`, and writes parsed rows to `perf-window.tsv`,
  `perf-window-aggregate.tsv`, and `perf-window-comparison.tsv`. Validation:
  `bash -n`, `git diff --check`, a no-perf vector2 shape smoke with `NA`
  aggregate rows, and a two-driver two-repeat 1 MiB host-to-guest TCP smoke
  with non-empty raw perf CSVs, median aggregate rows, comparison ratios, and
  no stale TAP/UML state, 2026-06-11. This is the right harness shape for the
  remaining steady-state syscall-rate work, but the short smoke is not itself
  the final P4.3 gate.
- The first focused 1 MiB host-to-guest run with UML process metrics confirms
  ...
  allocation alone.
- A follow-up five-cell 1 MiB host-to-guest parameter/topology sweep used the
  same helper with three repeats per driver and transfer-window perf
  collection.  Default fd/multiqueue vector2 remained below legacy with
  host-side median ratio 0.650602.  `TCP_NODELAY` and in-process TAP were also
  below the bar at 0.560706 and 0.639896.  A 16 KiB sender chunk did not close
  the cell because the apparent median win came from legacy-vector outliers
  and the best ratio stayed at 0.530945.  Single-queue fd mode was best at
  0.810510 median and 0.906732 best, pointing at queue selection and
  wakeup/receive scheduling policy while leaving the median gate open.
...
- Active launcher/selftest planning-label cleanup landed for `Cargo.toml`,
  ...
  gate TOMLs, `umlctl up --dry-run` for touched Umlfiles, and
  `git diff --check` all passed on 2026-06-10.
...
2. Vector2 replacement claims must match validation evidence. Current-head
   ...
   guest-to-host TCP gate, lazy-RX allocation-churn cleanup, RX checksum
   feature alignment, and most fixed-byte bidirectional TCP cells pass, but the
   host-to-guest 1 MiB regression, full KVM-v2 Tier 3 coverage, broader UDP
   plus syscall-rate/full CPU-utilisation performance coverage beyond the
   current endpoint and UML process deltas, and multiqueue fairness/performance
   gates remain open.
