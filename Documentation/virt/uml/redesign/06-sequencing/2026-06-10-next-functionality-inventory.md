...
| Record/replay | Strict unsupported syscall policy | Present-experimental-core | `next` | Initial R/R-1 strict replay accepts only `getpid`, `getppid`, `gettid`, and payload-aware `uname(2)`. Other syscalls fail closed with `-EOPNOTSUPP`, SIGSEGV, and debugfs failure counters rather than falling back to live execution or scalar-only replay. Broader syscall coverage must be added deliberately with payload/time/device policy. | `um_kvm_v2_record` strict replay failure accounting case PASS; dispatcher check builds on `next`, 2026-06-11. |
...
- KVM v2 experimental record/replay core is present on `next` behind
  ...
  the first supported copyout syscall, strict replay failure accounting, and
  ...
  `make ARCH=um -j16`, `um_kvm_v2_record` 15/15, `kvm_v2_marshal` 9/9,
