...
| Vector2 | performance benchmark harness | Present-tooling-needs-runs | `next` | Keep `net-bench` as an explicit operator-run gate; do not treat it as default selftest runtime because it needs a UML kernel, TAP setup, and controlled benchmark timing. | `tools/testing/selftests/um/net-bench` now has a kselftest Makefile, builds `tcp-send`, uses repo-relative defaults instead of developer-local paths, and validates wrapper syntax; no new throughput verdict is claimed, 2026-06-11. |
