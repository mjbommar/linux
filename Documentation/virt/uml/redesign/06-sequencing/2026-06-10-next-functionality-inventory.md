...
| Instrumentation | KFENCE | Present-profile-probed-needs-report-smoke | `next`, `research` and `fuzz-deep` profiles | Keep profile config and add a focused KFENCE report smoke before final completion. | Research-profile runtime probe confirms `/sys/kernel/debug/kfence/stats` present; the current KASAN `cve-repro` gate does not exercise KFENCE report generation, 2026-06-10. |
| Instrumentation | KCSAN | Present-needs-runtime-profile-gate | `next`, `race` profile | Build and boot the race profile with `ncpus=2`, validate `debugfs_kcsan`, and capture selftest/no-race-report evidence. | `race.config` selects `CONFIG_KCSAN=y`, `CONFIG_KCSAN_SELFTEST=y`, `CONFIG_SMP=y`, and disables KASAN by design; current runtime gate remains open, 2026-06-10. |
| Instrumentation | KCOV | Present-needs-runtime-fuzz-gate | `next`, `fuzz` and `fuzz-deep` profiles | Build a KCOV-enabled fuzz profile and run a focused `/sys/kernel/debug/kcov` mmap/ioctl smoke. | `fuzz` and `fuzz-deep` fragments enable `CONFIG_KCOV=y` and `CONFIG_KCOV_ENABLE_COMPARISONS=y`; `research` intentionally keeps KCOV off and the profile-check harness now expects `debugfs_kcov=ABSENT` there, 2026-06-10. |
