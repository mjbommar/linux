...
| Vector2 | failed-open fault injection | Present-documented-validation-only | `next`, `umlctl-deploy` | Keep as a validation-only open-unwind knob; leave unset for normal workloads. | `fail_open_after=N` is documented in kernel and umlctl comments plus the example README as validation-only; the live `vector2-failed-open` Umlfile remains the intended gate, 2026-06-10. |
