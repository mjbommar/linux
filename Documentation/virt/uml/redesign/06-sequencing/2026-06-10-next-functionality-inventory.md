...
| Instrumentation | BPF/JIT shims | Present-needs-profile-binary | `next` | Keep research-profile BPF/JIT support; runtime closure still needs a booted research binary plus guest `bpftool` or `bpftrace` smoke. | Clean temporary worktree `make ARCH=um O=<out> uml/research` confirms `CONFIG_HAVE_EBPF_JIT=y`, `CONFIG_BPF_SYSCALL=y`, `CONFIG_BPF_JIT=y`, and `CONFIG_BPF_JIT_ALWAYS_ON=y`; `make ARCH=um O=<out> arch/x86/net/bpf_jit_comp.o` PASS, 2026-06-10. |
