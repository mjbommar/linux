...
| Vector2 | performance benchmark harness | Present-tooling-gate-failing | `next` | Keep `net-bench` as an explicit operator-run gate; add TX batching before any replacement-ready claim. | `tools/testing/selftests/um/net-bench` now has a kselftest Makefile, builds `tcp-send`, uses repo-relative defaults instead of developer-local paths, and validates wrapper syntax. Initial TCP run: legacy vector 3/3 PASS median 39805.4 Mbps; vector2 3/3 PASS median 18949.8 Mbps; ratio 0.476 < 0.85. Scatter-gather TX fix improved vector2 to median 22953.7 Mbps versus legacy 40041.7 Mbps, ratio 0.573; still VERDICT FAIL, 2026-06-11. |
