...
| Instrumentation | KMSAN | Present-needs-runtime-fix | `next` | Keep `research-kmsan`; finish runtime initialization cleanup now that the LLVM profile build exists and the vmalloc metadata layout is fixed. | Clean LLVM `uml/research-kmsan` build PASS on rebuilt `df02aa4dd19b`, 2026-06-11. `kmsan-smoke` still FAILs before the result marker. A UML stacktrace cleanup now honors `trace->skip`, so the first report is attributed to the real site, `sized_strscpy()` in the kthread-name copy path, followed by scheduler/credential/string reports. The previous early `__vmap_pages_range_noflush()` / `vmalloc error` metadata-mapping failure remains absent. |
...
- UML KMSAN vmalloc metadata alignment is fixed on `next`: the quarter size
  ...
  report is attributed to `sized_strscpy()` in the kthread-name copy path
  rather than to `save_stack_trace()`. KMSAN remains open as a runtime blocker
  because follow-on scheduler, credential, and string-formatting reports remain.
