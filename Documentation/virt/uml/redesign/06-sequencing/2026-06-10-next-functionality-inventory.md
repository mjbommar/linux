...
| Pool | `umlctl exec` via daemon | Present-needs-fix | `next`, `memo09-phase4` | Keep; error envelope validated, but successful guest exec still needs a real kernel exec primitive or replacement transport. | `pool-exec-smoke` PASS for expected failure envelope; exec now waits for member mconsole `version` readiness and uses the native client, then reaches `mconsole command failed: Unknown command` because the active mconsole command table has no `exec` verb, 2026-06-10. |
