...
| Record/replay | Deterministic task-owned workload replay | Present-experimental-core | `next` | The task-owned helper now suppresses the debugfs control `write(2)` from the record log, records only the bounded scalar plus `uname(2)`, `getcwd(2)`, and direct raw-time payload workload, replays it from the snapshot-backed log, compares replayed raw-time bytes with the recorded values, and auto-disarms when the cursor reaches the end. This proves the first R/R-1 deterministic workload for the current direct-syscall subset, not whole-system replay. | `KVM_RECORD_TASK: PASS pid=1 entries=389 syscalls=389 same=389 other=0 payload_entries=5 payload_bytes=448 replayed=389`, 2026-06-11. |
