...
| Launcher | deploy configs | Present-needs-comparison | `next`, `umlctl-deploy` | Import missing useful configs only; current `umlctl` uses `up/down` plus Umlfile parsing, not a separate deploy subcommand. | Umlfile deploy path covered by `cargo test` plus `umlctl-smoke`; historical config comparison still pending, 2026-06-10. |
