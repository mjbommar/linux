...
- added the first R/R-1 syscall-payload model gate: `uname(2)` records and can
  ...
  set. `kvm-record-smoke` now reports `KUnit=24/24`, and the task-owned leg
  reports `payload_entries=5`, `payload_bytes=448`, and `replayed=389`; and
...
| Deterministic time-travel and record/replay as first-class | Experimental Kconfig-gated R/R-1 exists with task-owned snapshot start, scalar replay, `uname(2)`/`getcwd(2)` payload replay, task-owned direct raw-time payload replay, time-travel clock events, strict fail-closed unsupported syscall policy, timestamp-instruction blocking, SIGALRM blocking during replay, and device/randomness rejection. | Either finish and document R/R-1 as the supported first tier, or explicitly exclude record/replay from the final completion claim with approval. Broader workload coverage, supported-entry mismatch, signal-event ordering, and device/network/hostfs policy remain the main gaps. |
...
| S5 | Vector2 publication gates | The net-bench harness, scatter-gather TX cleanup, diagnostics, TX/RX NAPI scheduling fix, and lazy-RX cleanup are landed. The normal guest-to-host TCP gate now passes at ratio 0.993 against the 0.85 bar. The fixed-byte bidirectional TCP refresh closes most TCP cells but leaves a reproducible 1 MiB host-to-guest regression; the metric rerun keeps it open and points at vector2 scheduler/wakeup overhead, with about 12x the legacy vector scheduler pcount/context-switch deltas. UDP harness support and initial paced UDP evidence are landed. Next work is the rest of the publication matrix: natural seccomp long gate, full KVM-v2 Tier 3 networking, host-to-guest small-transfer follow-up, unpaced/larger UDP, syscall-rate, full CPU-utilisation, and multiqueue fairness/performance tests. | Vector2 KUnit, fd/multiqueue/inproc/pool/sandbox smokes, TCP net-bench ratio >= 0.85, Tier 3 reports, and fairness/performance report. |
...
- The task-owned helper now suppresses the debugfs control write from the
  ...
  direct raw-time payload workload, replays 389/389 entries from that log,
  compares replayed raw-time bytes with the recorded values, and verifies no
  other-task contamination.
...
7. **Deterministic workload selftest.**
  ...
  `kvm-record-smoke` now includes the task-owned 389-entry replay leg, live
...
4. Finish the remaining R/R-1 record/replay closure. Task-owned session start,
   ...
   supported-entry divergence coverage, selftest hardening, and documentation
   slices in order.
