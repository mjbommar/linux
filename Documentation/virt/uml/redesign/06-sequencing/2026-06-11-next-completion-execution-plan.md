...
- UDP fixed-byte coverage now exists in the same helper through
  ...
  guest-to-host also passed for both drivers, but 8 MiB host-to-guest still
  lost bytes before exact-byte completion for both legacy vector and vector2.
...
- Expand the remaining P4.3 measurements: larger host-to-guest UDP throughput,
...
2. Preserve the fixed vector2 TCP guest-to-host gate on the final stack. The
   ...
   host-to-guest small-transfer, larger host-to-guest UDP, syscall-rate, and full
...
3. Finish vector2 publication evidence after the 1 MiB host-to-guest
   fixed-byte gap and larger host-to-guest UDP gap are fixed or explicitly
   bounded: natural 7200-second seccomp Tier 3, full KVM-v2 Tier 3 networking
   coverage, and multiqueue fairness/performance. Keep v2 opt-in until those
   gates justify stronger language.
