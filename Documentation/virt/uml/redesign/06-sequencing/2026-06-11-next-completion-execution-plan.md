...
- aligned the vector2 fake-host RX path with production fd/TAP lazy batching:
  ...
  it does not close the throughput blocker.
...
| Vector2 publication readiness | Vector2 has strong focused smoke evidence, and the measured guest-to-host TCP regression now passes the 0.85 gate after the TX/RX NAPI scheduling fix: vector2 median 37116.0 Mbps versus legacy 40080.5 Mbps, ratio 0.926. Lazy RX preserves that gate at ratio 0.993 and removes the worst RX-slot over-preparation, but it does not close the small-transfer host-to-guest regression. The fake-host KUnit backend now mirrors production fd/TAP lazy RX batching, so tests cover the one-slot speculative read/release shape used by the open host-to-guest investigation. The fixed-byte bidirectional TCP refresh clears guest-to-host at 1/8/32 MiB and host-to-guest at 8/32 MiB, but the 1 MiB host-to-guest metric run still misses the no-regression bar: host-side median ratio is 0.9147 and best ratio is 0.6656, with vector2 producing about 12x the legacy vector scheduler pcount/context-switch deltas. Initial paced UDP evidence exists, buffered unpaced 1 MiB UDP now passes with vector2/vector host-side ratios of 0.963217 guest-to-host and 0.985371 host-to-guest, buffered 8 MiB guest-to-host passes for both drivers, and paced 8 MiB host-to-guest passes with vector2/vector host-side ratio 0.998875. A first privileged perf-stat smoke proves syscall/CPU collection is available per driver, and the helper now emits bounded host-to-guest UML-PID `perf-window.tsv` rows, but steady-state CPU/syscall coverage remains open. Unpaced larger host-to-guest UDP still loses bytes before exact-byte completion. Replacement/publication still needs the rest of P4.3 plus long-run and fairness evidence. | Finish the natural seccomp long run, equivalent KVM v2 Tier 3 networking, host-to-guest small-transfer follow-up using the new fixed-byte diagnostics, final paced/unpaced UDP acceptance shape, steady-state syscall-rate, full CPU-utilisation, and multiqueue fairness/performance evidence. Keep parser-only transports out of runtime claims and keep vector2 opt-in until the evidence justifies stronger language. |
...
| S5 | Vector2 publication gates | The net-bench harness, scatter-gather TX cleanup, diagnostics, TX/RX NAPI scheduling fix, lazy-RX cleanup, and fake-host lazy RX fidelity gate are landed. The normal guest-to-host TCP gate now passes at ratio 0.993 against the 0.85 bar, and `um_vector2_*` KUnit now covers the production-style one-slot speculative RX read/release shape. The fixed-byte bidirectional TCP refresh closes most TCP cells but leaves a reproducible 1 MiB host-to-guest regression; the metric rerun keeps it open and points at vector2 scheduler/wakeup overhead, with about 12x the legacy vector scheduler pcount/context-switch deltas. UDP harness support, initial paced UDP evidence, buffered unpaced 1 MiB UDP, buffered 8 MiB guest-to-host UDP, paced 8 MiB host-to-guest UDP evidence, an initial privileged perf-stat smoke, and bounded `perf-window.tsv` support are landed; unpaced larger host-to-guest UDP remains open. Next work is the rest of the publication matrix: natural seccomp long gate, full KVM-v2 Tier 3 networking, host-to-guest small-transfer follow-up, final paced/unpaced UDP acceptance shape, steady-state syscall-rate, full CPU-utilisation, and multiqueue fairness/performance tests. | Vector2 KUnit, fd/multiqueue/inproc/pool/sandbox smokes, TCP net-bench ratio >= 0.85, Tier 3 reports, and fairness/performance report. |
...
- UDP fixed-byte coverage now exists in the same helper through
  ...
  a TCP compatibility smoke, a vector2 UDP bidirectional 64 KiB smoke, a paced
  1 MiB UDP matrix, and a buffered unpaced 1 MiB UDP matrix.  In the buffered
  unpaced 1 MiB run, vector2/vector host-side ratios were 0.963217
  guest-to-host and 0.985371 host-to-guest.  Buffered unpaced 8 MiB
  guest-to-host also passed for both drivers.  Unpaced 8 MiB host-to-guest
  still lost bytes before exact-byte completion for both legacy vector and
  vector2, but the paced 20 usec 8 MiB host-to-guest run passed for both
  drivers with vector2/vector host-side ratio 0.998875.
...
- The fixed-byte helper now supports an opt-in host-to-guest transfer-window
  ...
  parsed counters to `perf-window.tsv`.  A two-driver 64 KiB TCP smoke
  completed with non-empty raw perf CSVs: legacy vector recorded 85,743 total
  syscalls, 1,195.24 ms task-clock, and 9,895 context switches; vector2
  recorded 65,186 total syscalls, 984.71 ms task-clock, and 6,582 context
  switches.  This is closer to the final measurement shape because it excludes
  most boot/setup cost, but it remains a short smoke rather than a
  same-throughput steady-state comparison.
...
- Follow up the host-to-guest 1 MiB fixed-byte regression with the new
  ...
  the next bottleneck pass should look beyond raw RX allocation churn. The
  first metric run points specifically at wakeup/readiness/receive scheduling:
  vector2 produced about 12x the median scheduler pcount and voluntary context
  switches seen with legacy vector.
- Expand the remaining P4.3 measurements: final paced/unpaced UDP acceptance
  shape, steady-state syscall rate, and full CPU utilisation against legacy
  vector.  The new fixed-byte endpoint CPU columns, host-to-guest UML process
  deltas, and `perf-window.tsv` rows are diagnostic aids and measurement
  scaffolding, not substitutes for that acceptance gate.
...
2. Preserve the fixed vector2 TCP guest-to-host gate on the final stack. The
   ...
   host-to-guest small-transfer, final paced/unpaced UDP acceptance shape,
   syscall-rate, and full CPU-utilisation evidence. The helper-level endpoint
   CPU timing is now
...
3. Finish vector2 publication evidence after the 1 MiB host-to-guest
   fixed-byte gap and the paced/unpaced UDP acceptance shape are fixed or
   explicitly bounded: natural 7200-second seccomp Tier 3, full KVM-v2 Tier 3
