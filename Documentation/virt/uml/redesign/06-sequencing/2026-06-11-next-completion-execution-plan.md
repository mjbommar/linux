...
4. **Time policy.**
   ...
   log. Live smoke coverage now proves replay-mode `RDTSC` faults; replayable
   raw-time payloads and direct `RDTSCP` runtime evidence remain future work.
...
7. **Deterministic workload selftest.**
   ...
   `kvm-record-smoke` now includes the task-owned 386-entry replay leg,
   live raw-time and RDTSC rejection legs, plus a live-negative `getrandom(2)`
   strict rejection leg.
