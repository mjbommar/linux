...
- Follow up the host-to-guest 1 MiB fixed-byte regression with the new
  ...
  state, RX-reschedule probes reduced churn without closing throughput, and
  the parameter/topology sweep shows single-queue fd is the best current hint
  without a median closure.  The next bottleneck pass should look beyond raw
  RX allocation churn and fd handoff into queue selection, wakeup/readiness,
  and receive scheduling. The first metric run points specifically at
  wakeup/readiness/receive scheduling: vector2 produced about 12x the median
  scheduler pcount and voluntary context switches seen with legacy vector.
