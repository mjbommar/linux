...
- KVM v2 record/replay has an experimental core on `next` behind
  ...
  `uname(2)`/`getcwd(2)` payload workload and multiple direct raw-time
  argument shapes for `clock_gettime(2)`, `gettimeofday(2)`, and `time(2)`
  without other-task contamination.
  ...
  number and arguments match. They can also replay `gettimeofday(2)`'s
  optional returned `struct __kernel_old_timeval` and `struct timezone` bytes,
  plus `time(2)`'s scalar return and optional stored `__kernel_old_time_t`
  value. Strict replay now fails closed and records a failure counter for
  syscalls outside the initial bounded replay subset:
  `getpid`, `getppid`, `gettid`, payload-aware `uname(2)`/`getcwd(2)`, and
  payload-aware `clock_gettime(2)`/`gettimeofday(2)`/`time(2)`. Replay mode
  ...
  them as scalar-only entries. The live raw-time smoke now proves replay-mode
  direct syscall and current UML vDSO wrapper calls to `clock_gettime(2)`,
  `gettimeofday(2)`, and `time(2)` return recorded time bytes instead of
  observing host time outside the log. UML's current vDSO wrappers deliberately
  enter the syscall path so UML can trap them.
  A live supported-syscall mismatch
  ...
  `Documentation/virt/uml/kvm-v2-record-replay.rst` documents the supported
  experimental determinism tier and current non-goals. Broader payload
  coverage, replayable signal-event ordering,
  device/network/hostfs event replay, and full deterministic replay policy
  remain incomplete.
...
- Experimental record/replay KUnit: `um_kvm_v2_record` passes 24/24 with
  ...
  preservation, divergence cursor preservation, `uname(2)`, `getcwd(2)`,
  `clock_gettime(2)`, `gettimeofday(2)`, and `time(2)` payload restore,
  too-small payload buffer rejection, payload argument mismatch, payload
  overflow accounting, strict replay syscall-policy coverage, bounded
  raw-time syscall policy,
...
- Experimental task-owned record smoke: the static `kvm-record-task` helper
  ...
  requires `syscalls_from_other_tasks=0`, records eight payload entries
  totaling 528 bytes, then replays the same 393-entry task-owned workload from
  ...
  `getcwd(2)`, two `clock_gettime(2)` clock ids, multiple `gettimeofday(2)`
  output-pointer shapes, and pointer plus `NULL` `time(2)` calls, and it
...
- Experimental strict mismatch smokes: the static `kvm-record-mismatch`
  helpers record a supported payload-aware entry, arm replay, then issue the
  same syscall with mismatched replay arguments. The covered live divergence
  cases are `getcwd(2)` buffer-size mismatch, `clock_gettime(2)` clock-id
  mismatch, `gettimeofday(2)` output-pointer-shape mismatch, and `time(2)`
  scalar-versus-payload mismatch. The host runner treats the expected
  init-killing SIGSEGV as PASS only when the kernel logs strict replay
  divergence for the supported syscall (`nr=79`, `nr=228`, `nr=96`, and
  `nr=201` on the validated x86_64 run).
- Experimental raw-time payload smoke: the static `kvm-record-time` helper
  records one direct-syscall and one UML-vDSO-wrapper `clock_gettime(2)`
  result, one direct-syscall and one UML-vDSO-wrapper `gettimeofday(2)` result,
  and one direct-syscall and one UML-vDSO-wrapper `time(2)` result. It arms
  replay, consumes the recorded entries, and verifies the replayed
  `struct timespec`, `struct timeval`/`struct timezone`, and `time(2)` bytes
  match the recorded values. The `kvm-record-smoke` summary now reports
  `live-time=1` for replayed raw-time payload bytes rather than fail-closed
  rejection.
...
- Public record/replay determinism-tier documentation:
  ...
  fail-closed behavior, validation command, and non-goals.
...
The strongest long-run vector2 evidence is now the natural current-branch
seccomp Tier 3 run:

- 7221 seconds against the planned 7200-second window;
- 740/740 passes, with no failures or timeouts;
- Django vector2/seccomp: 370/370 passes;
- FastAPI vector2/seccomp: 370/370 passes;
- all scoreboard rows recorded vector2, `vec2.0`, seccomp, TAP transport,
  in-process host mode, and a single queue;
- all 740 per-run logs reached `SERVER_READY`,
  `GUEST_CURL ok=100 fail=0`, `TIER3_OK`, and `REPRO_DONE rc=0`;
- the run-log scan returned no fatal, panic, warning, KCSAN/data-race, or
  Tier 3 failure signatures; and
- teardown left no matching soak process or stray soak/vector2 TAP device.

The previous stopped-clean seccomp Tier 3 run remains historical supporting
evidence:
...
Post-upstream-merge sanity on `b1c2aad96149` adds:

- `next` includes `torvalds/master` through `2b414a95b8f7`;
- `make ARCH=um olddefconfig` reported no `.config` change;
- `make ARCH=um -j"$(nproc)" linux` completed and linked the UML binary;
- `um_vector2_*` KUnit on the rebuilt binary passed with 96 pass, 0 fail, and
  2 expected trusted-TAP skips; and
- the saved KUnit log scan returned no `not ok`, panic, BUG, WARNING, KCSAN,
  data-race, or FAILED markers.
...
- A follow-up lazy-RX batch cleanup prepares one receive slot per nonblocking
  read attempt instead of preparing the full NAPI budget up front.  It keeps
  the normal guest-to-host TCP gate green with legacy vector median
  39215.2 Mbps, vector2 median 38960.1 Mbps, and ratio 0.993, and it reduces
  representative 1 MiB host-to-guest RX-slot churn from 32448 prepared / 906
  received / 31542 released slots to about 1410 prepared / 905 received / 505
  released slots.  It does not close the 1 MiB host-to-guest fixed-byte
  throughput gap.
- A follow-up RX checksum feature-alignment slice makes vector2 advertise
  fixed RX checksum support when `csum=1`, matching the vnet-header receive
  path and legacy vector's TAP behavior.  Validation reports
  `um_vector2_*` KUnit 89 pass, 0 fail, 2 trusted-TAP skips; fd handoff,
  fd multiqueue, and in-process TAP smokes PASS; the fixed-byte diagnostic now
  shows vector2 `rx-checksumming: on [fixed]` with `vnet_hdr_enabled: 1`; and
  the normal guest-to-host TCP gate remains green with legacy vector median
  39854.4 Mbps, vector2 median 39441.7 Mbps, and ratio 0.990.
- A follow-up fd/vnet RX allocation-alignment slice makes NAPI RX allocation
  follow the channel's actual `vnet_hdr` runtime state instead of the transport
  enum, so launcher-owned fd TAP channels with `IFF_VNET_HDR` allocate space
  for the virtio header.  Validation reports `um_vector2_*` KUnit 90 pass, 0
  fail, 2 trusted-TAP skips; fd handoff, fd multiqueue, and in-process TAP
  smokes PASS; and a focused 1 MiB host-to-guest diagnostic shows vector2
  median 0.895 MiB/s versus legacy vector median 0.976 MiB/s.  This closes a
  correctness bug, not the small-transfer publication blocker.
- The fixed-byte performance helper now supports explicit TCP or UDP runs.
  TCP remains the default; UDP adds fixed-byte sender/sink paths, a datagram
  payload knob, optional inter-datagram pacing, requested UDP receive/send
  buffer knobs defaulting to 4 MiB, fail-fast handling when a guest exits
...
- The fixed-byte performance helper now samples `umlctl metrics --json` around
  host-to-guest transfers and appends UML host-process deltas for user/system
  CPU seconds, scheduler run/wait seconds, scheduler pcount, and voluntary/
  involuntary context switches.  Validation reports `bash -n`,
  `git diff --check`, vector2 TCP 64 KiB host-to-guest smoke PASS with real
  UML process metric deltas, a side-by-side legacy vector/vector2 TCP 64 KiB
  host-to-guest smoke PASS with 22-field summary rows for both drivers,
  vector2 TCP 64 KiB guest-to-host shape smoke PASS with `NA` metric fields,
  and cleanup checks with no stale `vperf-*` TAPs or live `umlctl` instances.
  This gives the next host-to-guest bottleneck pass repeatable unprivileged
  CPU/scheduler diagnostics.  It does not close the
...
- The vector2 fake-host KUnit backend now mirrors the production fd/TAP lazy
  RX allocation shape.  It prepares one RX slot per attempted read, releases
  one speculative probe slot on empty or limited reads, and keeps zero-packet
  batch accounting separate from partial progress.  Validation reports
  `um_vector2_fake_host` KUnit 10/10 PASS and `um_vector2_*` KUnit 94 pass, 0
  fail, 2 trusted-TAP skips on the rebuilt current tree.  This tightens
  coverage for the RX path used by the host-to-guest investigation; it does
  not close the 1 MiB throughput blocker.
- A bounded 1 MiB host-to-guest TCP parameter/topology sweep ran three repeats
  for each driver with transfer-window perf collection enabled.  Default
  fd/multiqueue vector2 still trailed legacy vector with a host-side median
  ratio of 0.650602.  `TCP_NODELAY` and in-process TAP did not close the cell,
  at median ratios of 0.560706 and 0.639896.  A 16 KiB sender chunk produced a
  misleading median win because legacy vector had two slow outliers; its best
  vector2/vector ratio was only 0.530945.  The strongest hint is single-queue
  fd mode: median ratio 0.810510 and best ratio 0.906732, still short of a
...
- A focused privileged `perf stat` smoke now validates that syscall-rate and
  ...
  host-to-guest run per driver passed through the fixed-byte helper.  The
  subtree-wide perf CSVs reported 367 syscall tracepoints per run: legacy
  vector recorded 4,294,127 total syscalls, 111,886.52 ms task-clock, and
  152,987 context switches; vector2 recorded 510,383 total syscalls,
  8,725.39 ms task-clock, and 71,373 context switches.  This is setup, boot,
  transfer, and teardown smoke evidence, not the final steady-state
...
- active soak templates no longer carry developer-local kernel, `umlctl`,
  helper, or virtualenv paths. `run-soak-daemon.sh` and `run-pilot.sh` now
  render repo-relative `{{SOAK_DIR}}`/`{{REPO_ROOT}}` placeholders, default to
  the repo-built debug `umlctl` when present, expose `TIER2_UV_PYTHON` for the
  optional uv virtualenv, and pass Bash syntax plus a dry-run render over
  memcheck, kbuild-tiny, tier2 uv, and tier3 vector2 templates.
- `Documentation/virt/uml/review-01/` now contains only compatibility
  pointers. The historical review and response moved to
  `Documentation/virt/uml/redesign/07-references/review-01/`, with archive
  notices and updated redesign cross-references, so workstream review prose no
  longer lives in the active non-redesign UML documentation surface.
...
scalar-only entries. Direct syscall and current UML vDSO wrapper calls to raw
`clock_gettime(2)`, `gettimeofday(2)`, and `time(2)` are now replayed from
logged payload bytes, while broader raw-time interfaces remain outside the
bounded tier. Replay mode sets CR4.TSD so direct user timestamp reads fault. The
...
The task-owned smoke now proves deterministic replay for the bounded 393-entry
scalar plus `uname(2)`, `getcwd(2)`, and direct raw-time argument-shape
payload workload, and the mismatch smokes prove that supported `getcwd(2)`,
`clock_gettime(2)`, `gettimeofday(2)`, and `time(2)` replay entries with
mismatched replay arguments kill the guest instead of falling back. The
...
- vector2 has a steady-state syscall-rate and CPU-utilisation comparison, not
  just endpoint timing, `umlctl metrics` deltas, or the initial perf-stat smoke;
