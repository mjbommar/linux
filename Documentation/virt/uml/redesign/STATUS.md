...
| SMP-T57 | stress-ng `--vm --verify` SIGILL on kvm-v2 — narrowed via 5 probe cycles (memo state-audit/24). Cycles 1-3: IST-write + SYSRETQ-RCX RULED OUT. Cycle 4 (§7.6-D): GPR drift RULED OUT. Cycle 5 (§7.6-F): user-stack TDP coherence RULED OUT — saved RIP/RBP at user RSP bit-stable across 100 #PFs and the UD; UD's user_rsp is the SAME as in-loop (not a chain of call frames as §7.6-D inferred). The UD is forward instruction-fetch corruption mid-function, not a `ret`-with-wrong-saved-RIP. New PRIMARY hypothesis (P=0.55): FPU/XSAVE residue across the per-vCPU FPU-dirty-epoch flag (T55 territory). New cheap hypothesis (P=0.30): user TEXT-page TDP mis-mapping — at UD `ip=0x550000e9d71e`, kernel reads user-instruction bytes; if they don't match `objdump -d /usr/bin/stress-ng`, text-page is stale. | OPEN (narrowing) | task #243; memo state-audit/24. Next probes: §7.6-H (text-page check, cheap), §7.6-E (FPU log, primary), §7.6-G (disable KVM_CAP_SYNC_REGS). |
