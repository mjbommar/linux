...
| SMP-T57 | stress-ng `--vm --verify` SIGILL on kvm-v2 — re-localised by §6 probes (memo 24 §7). Single-vCPU still FAILs (per-vCPU IST stale-frame ruled out, §5.2). 5791 `kvm_v2_ist_frame_write` events captured; only one had non-zero delta and it was a legitimate SIGILL handler delivery — IST-write path is innocent. New top hypothesis: SYSRETQ-RCX corruption on syscall-marshal-out at `kvm_v2_handle_io_trap` lines 2367-2369 (P=0.45). 22/38 vm-methods FAIL, 16/38 PASS. | OPEN (re-localised) | task #243; memo state-audit/24 §7. Next probe: instrument syscall marshal-out per §7.6. |
