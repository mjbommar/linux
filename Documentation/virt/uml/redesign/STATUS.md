...
| J     | validation — 24h continuous + Tier 1/2/3 + LTP | **IN PROGRESS** | task #167. Daemon-mode wrapper landed `4977e0357ce5` (`run-soak-daemon.sh`, 485 LoC, smoke-tested 100s real run); Tier 1/2/3 + LTP still needed. Design memo `phase-J-design-2026-05-07.md` §3-4 specifies remaining work. |
...
| SMP-T57 | stress-ng `--vm --verify` SIGILL on kvm-v2 — **CHARACTERISED at root-cause level (memo state-audit/24 §7.6-H, P=0.85)**: the SIGILL fires on `vpxor %xmm6,%xmm6,%xmm6` (VEX-encoded AVX). User-instruction bytes at the SIGILL site MATCH `objdump -d /usr/bin/stress-ng` byte-for-byte across 3 boots. A valid AVX opcode raising #UD on an AVX-supporting host = `XCR0.YMM` cleared OR `CR4.OSXSAVE` cleared OR XSAVE-header `xstate_bv` missing YMM at decode time. Text-page TDP RULED OUT. Five prior cycles (IST-write, SYSRETQ-RCX, GPR drift, user-stack TDP, text-page TDP) all RULED OUT. The narrowing is sufficient to write the fix without further probes; §7.6-E logging would add corroboration but is not on the critical path. | OPEN (root-cause level) — **fix design ready** | task #243; memo state-audit/24 §7.6-H. Fix territory: XCR0 / CR4.OSXSAVE preservation across per-vCPU dispatch boundaries — SMP-T55 (D119) per-vCPU FPU-dirty epoch is adjacent, may need extension to cover XSAVE-header / XCR0. |
