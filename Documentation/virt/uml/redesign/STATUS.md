...
| SMP-T57 | stress-ng `--vm --verify` SIGILL on kvm-v2 — narrowed via 3 probe cycles (memo state-audit/24). Cycle 1 (§3 matrix): 22/38 methods FAIL with same `SIGILL si_addr=0` signature. Cycle 2 (§7): IST-write path RULED OUT — single-vCPU still FAILs, all 5791 ist_frame_write events preserved RIP. Cycle 3 (§7.6 follow-up): SYSRETQ-RCX corruption + rt_sigreturn-via-IST RULED OUT — failing pid traversed 0 SYSCALL events. New leading hypothesis: GPR/XSAVE state leak on the #PF marshal-in/out round-trip (memo §7.6-D, P=0.70); user falls through mscan loop epilogue with corrupted register/FPU state, lands mid-instruction at +0x6f. | OPEN (narrowing) | task #243; memo state-audit/24. Next probes: (a) §7.6-D GPR/XSAVE log on PF marshal entry/exit, (b) cheap orthogonal — rebuild stress-ng without AVX/SSE4. |
