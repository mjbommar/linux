...
| SMP-T57 | stress-ng `--vm --verify` SIGILL on kvm-v2 — narrowed via 4 probe cycles (memo state-audit/24). Cycles 1-3 (§3, §7, §7.6): IST-write + SYSRETQ-RCX both RULED OUT. Cycle 4 (§7.6-D): all 16 GPRs + FS_BASE + GS_BASE preserved bit-exactly across 4096 #PFs — GPR drift RULED OUT. State at UD time is legitimately post-loop (r9=0x1000000 = exact 16 MiB). New leading hypothesis (P=0.45): TDP / page-table coherence corruption on the user stack near `ret` after loop epilogue; user stack saved-RIP read returns wrong address. XSAVE/FPU residue still suspect at P=0.35. | OPEN (narrowing) | task #243; memo state-audit/24. Next probes: §7.6-F (cheap — `__get_user` saved RIP from kernel), §7.6-E (FPU log), §7.6-G (disable KVM_CAP_SYNC_REGS). |
