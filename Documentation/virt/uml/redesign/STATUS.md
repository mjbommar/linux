...
- KVM v2 record/replay has an experimental core on `next` behind
  ...
  number and arguments match. They can also replay `time(2)`'s scalar return
  and optional stored `__kernel_old_time_t` value. Strict replay now fails
  closed and records a failure counter for syscalls outside the initial
  bounded replay subset:
  `getpid`, `getppid`, `gettid`, payload-aware `uname(2)`/`getcwd(2)`, and
  payload-aware `clock_gettime(2)`/`time(2)`. Replay mode sets CR4.TSD so
  direct user timestamp reads fault instead of observing host time outside the log. The
  ...
  replay-mode `clock_gettime(2)` and `time(2)` syscalls return recorded time
  bytes instead of observing host time outside the log. A live supported-syscall mismatch
...
- Experimental record/replay KUnit: `um_kvm_v2_record` passes 23/23 with
  ...
  `clock_gettime(2)`, and `time(2)` payload restore, too-small payload buffer
  rejection, payload argument mismatch, payload overflow accounting, strict
  replay syscall-policy coverage, bounded raw-time syscall policy,
...
- Experimental raw-time payload smoke: the static `kvm-record-time` helper
  records one `clock_gettime(2)` result and one `time(2)` result, arms replay,
  consumes the recorded entries, and verifies the replayed `struct timespec`
  and `time(2)` bytes match the recorded values. The `kvm-record-smoke`
  summary now reports `live-time=1` for replayed raw-time payload bytes rather
  than fail-closed rejection.
...
- finish raw-time coverage beyond syscall `clock_gettime(2)`/`time(2)`,
  replayable asynchronous signal-event ordering, device/network/hostfs event
  policy, and broader deterministic workload coverage before counting the
  original record/replay mission complete;
...
scalar-only entries. Raw `clock_gettime(2)` and `time(2)` are now replayed
from logged payload bytes, while broader raw-time interfaces remain outside
the bounded tier. Replay mode sets CR4.TSD so direct user timestamp reads fault. The
...
`clock_gettime(2)` and `time(2)` return recorded time bytes rather than
observing host time outside the log. Additional raw-time interfaces, explicit
